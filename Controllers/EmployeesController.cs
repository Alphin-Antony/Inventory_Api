...
namespace Project_Api.Controllers
{
    ...
    public class EmployeesController : ControllerBase
    {
        ...
        {
            AddtoCartDto cart = new AddtoCartDto();
            if (!ModelState.IsValid)
            {
            ...
            try
            {
                // Find the existing AddtoCart entity based on the OrderNo
                var existingEntity =await dbContext.AddtoCarts.FirstOrDefaultAsync(e => e.OrderNo == updateCartDto.OrderNo);

                if (existingEntity == null)
                {
                    // If the entity is not found, return a 404 Not Found response
                    return NotFound($"No cart found for OrderNo: {updateCartDto.OrderNo}");
                }

                // Update the CartStatus of the existing entity
                existingEntity.CartStatus = updateCartDto.CartStatus;

                // Save the changes to the database
                await dbContext.SaveChangesAsync();

                return Ok(new { message = "Order Placed", stat = 1 });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> AddProduct([FromBody] Addproductdto addproductdto)
        {
            ...
            {
                // Map the DTO to the AddtoCart entity
                var addToCartEntity = new AddProducts
                {
                    ItemName = addproductdto.ItemName,
                    Price = addproductdto.Price,
                    ItemCount = addproductdto.ItemCount,
                ...
                dbContext.Add(addToCartEntity);
                await dbContext.SaveChangesAsync();
                return Ok(new { message = "Product added successfully", stat = 1 });
        ...
        [HttpGet("Getorders")]
        public async Task<IActionResult> GetItemsbyid11([FromQuery]string email,int status)
        {
            // Fetch items where CustId (assumed to be string) matches the given email
            var items = await dbContext.AddtoCarts
               .Where(i => i.CustId == email && i.CartStatus==status)  // Ensure CustId is a string
        ...
        public async Task<IActionResult> Getorderbyid(string email)
        {
            // Fetch items where CustId matches the given email
            var items = await dbContext.AddtoCarts
               .Where(i => i.CustId == email && i.CartStatus == 0)  // Ensure CustId is a string or compare correctly if not
